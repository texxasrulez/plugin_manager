...
return function (array $context): array {
    ...
    if ($pluginPath === '' || !is_dir($pluginPath)) {
        return array(
            'ok' => false,
            'message' => 'Additional SMTP maintenance review could not locate the plugin directory.',
            'details' => array(),
        );
    }
    ...
    if (is_dir($sqlDir)) {
        $details[] = 'SQL/ detected. Manual installs require importing the schema.';
    }
    ...
    $details[] = 'After enabling the plugin, review additional SMTP identities in Settings > Identities.';

    return array(
        'ok' => true,
        'message' => 'Additional SMTP maintenance review completed.',
        'details' => $details,
    );
};
