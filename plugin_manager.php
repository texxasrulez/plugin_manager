...
class plugin_manager extends rcube_plugin
{
    ...
    function render_page()
    {
        ...
        foreach ($rows as $r) {
            ...
            $is_outdated = ($st === $this->gettext('update_available') || strpos($st_raw, 'update') !== false);
            $is_up_to_date = ($st === $this->gettext('up_to_date') || strpos($st_raw, 'up to date') !== false);
            $is_bundled = ($st === $this->gettext('bundled') || $st_raw === strtolower($this->gettext('bundled')));
            $has_error = !$is_outdated && !$is_up_to_date && !$is_bundled;
